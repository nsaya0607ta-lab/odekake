...
const ITEM_SPAWN_WEIGHTS: Partial<Record<string, number>> = {
   ...
   * MR:212(ブレブル1種、ランク比率通り)。在籍分の実際の合計は1372（残り748は
   * `TIME_BONUS_UNFILLED_RANK_DOG_WEIGHT`でdogへ）。2026-09-03、おかえりの個別チューニング値(300)を
   * 廃止しランク比率に統合（ユーザー指定）。ただし統合直後の素の予算600だとLv5平均プレイ時間が
   * 181秒→95秒まで落ち込んだため、目標値（docs/item-catch-new-item-checklist.md）に戻すため
   * プール予算自体を600→2120に約3.5倍引き上げて再チューニング済み（`node
   * scripts/simulate-item-catch.mjs 20000 avoid`でLv5平均174.7秒・500秒超え0%を確認）。
   * 2026-09-03、ブレブルを出現量アップ・制御系プールからこちらに移設（ユーザー指定。「UR/LRのみ
   * 出現」の出現量制御スキルから「+25〜29秒」の時間増加スキルに変更したため）。MRランク比率通り
   * (212)を割り当てるとLv5平均プレイ時間が179秒→215秒（+20%）まで膨らむが、この上振れは
   * ユーザー確認済みで許容している（ブレブルだけ個別チューニングせず、他ランクと同じ「ランク予算を
   * 均等割り」のルールに素直に従わせる方針）。
   */
  other_omojii: 110,
  toy_duck_plush: 230,
 ...
 * 各値は「プール予算 − 在籍ランクの実際の重み合計」。時間増加系: 2120−1372=748 /
 ...
 * 時間増加系プールのMR枠にランク比率通り(212)で移設したため、時間増加系側は960→748に更新。
 * 出現量アップ制御系側はブレブル跡地をXmas Partyが埋めるため110のまま変更なし）。新しく
 * 未充填ランクにアイテムを追加したら、対応する定数からそのランクの予算分を差し引くこと。
 */
