...
const ITEM_SPAWN_WEIGHTS: Partial<Record<string, number>> = {
   ...
   * MR:212(ブレブル、ランク比率通りの均等割りではなく50に個別チューニング。理由は下記)。
   * 在籍分の実際の合計は1210（残り910は`TIME_BONUS_UNFILLED_RANK_DOG_WEIGHT`でdogへ）。2026-09-03、
   * おかえりの個別チューニング値(300)を廃止しランク比率に統合（ユーザー指定）。ただし統合直後の
   * 素の予算600だとLv5平均プレイ時間が181秒→95秒まで落ち込んだため、目標値
   ...
   * 174.7秒・500秒超え0%を確認）。2026-09-03、ブレブルを出現量アップ・制御系プールから
   * こちらに移設（ユーザー指定。「UR/LRのみ出現」の出現量制御スキルから「+25〜29秒」の時間増加
   * スキルに変更したため）。移設直後、MRランク比率通りの均等割り(212→210)をそのまま割り当てると
   * Lv5平均プレイ時間が179秒→215秒（+20%）まで膨らんだ（ブレブルは他の時間増加系より1回あたりの
   * 秒数が大きく(25〜29秒)、かつ出現重みも大きいため、プール総予算を変えないランク比率均等割りでは
   * 相殺しきれない）。目標値に戻すため、ブレブル1種のみ他ランクと異なりMR予算(212)ではなく50に
   * 個別チューニングし、余った162は`TIME_BONUS_UNFILLED_RANK_DOG_WEIGHT`に上乗せしてdogへ
   * 消化する（960→910。実測`node scripts/simulate-item-catch.mjs 20000 avoid`で確認、下記）。
   */
  other_omojii: 110,
  toy_duck_plush: 230,
  toy_carrot: 230,
   ...
   * SSR:144→40ずつ(4種) / UR:126(未充填→dog) / LR:108→110(1種) / MR:90→90(1種:Xmas Party)。
   * 在籍分の実際の合計は790（残り110は`SPAWN_DYNAMICS_UNFILLED_RANK_DOG_WEIGHT`でdogへ）。
   * 2026-09-03、ブレブルを時間増加系プールへ移設したため、MRランクはXmas Party単独になり
   * 90を丸ごと受け取る（ブレブル移設前も合わせてMR実額90は変わらないため、プール合計790・
   * 未充填分110自体は変更なし）。
   */
  toy_rainbow_ball: 40,
 ...
 * 各値は「プール予算 − 在籍ランクの実際の重み合計」。時間増加系: 2120−1210=910 /
 * 得点倍率系: 400−250=150 / 出現量アップ制御系: 900−790=110（2026-09-03時点、おかえりの
 * 個別チューニング値を廃止してランク比率のみに統合し、時間増加系プールの予算を600→2120に
 * 引き上げて目標プレイ時間を再現した後の値。さらに同日、ブレブルを出現量アップ制御系プールから
 * 時間増加系プールのMR枠に移設したため、時間増加系側は960→910に更新（ブレブルはランク比率通りの
 * 212ではなく50に個別チューニングしてあるため、通常の「新規充填ランクの予算分を丸ごと差し引く」
 * 計算にはならない。詳細はITEM_SPAWN_WEIGHTS直上のコメント参照）。出現量アップ制御系側は
 * ブレブル跡地をXmas Partyが埋めるため110のまま変更なし）。新しく未充填ランクにアイテムを追加したら、
    ...
    const weightedItems = itemPool.map((item) => ({
      ...
      weight:
        ...
        (otherSuppressActive && item.id !== STRETCH_ROD_ITEM_ID && OTHER_CATEGORY_ITEM_IDS.has(item.id)
          ...
          : 1) *
        (treasureRareLockActive && !HIGH_RARITY_LOCK_RARITIES.has(item.rarity) ? 0 : 1) *
