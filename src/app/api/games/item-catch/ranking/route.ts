...
export async function GET(request: Request) {
  ...
  const rpc = supabase.rpc.bind(supabase) as unknown as (
    fn: "get_friend_item_catch_ranking",
    args: { p_period: RankingPeriod },
  ) => Promise<RpcResponse>;

  const { data, error } = await rpc("get_friend_item_catch_ranking", { p_period: period });
  if (error) {
    ...
    console.error("Failed to load item catch friend ranking", {
      code: error.code,
