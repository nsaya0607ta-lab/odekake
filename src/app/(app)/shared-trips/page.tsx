...
import { leaveSharedTripAction, respondSharedTripInvitationAction } from "./actions";
...
export default async function SharedTripsPage({ searchParams }: { searchParams: Promise<{ created?: string; responded?: string; left?: string; error?: string }> }) {
  ...
  return (
    <>
      ...
      <PageBody>
        ...
        {params.left === "1" ? <p className="rounded-2xl border border-leaf bg-leaf-soft px-4 py-3 text-sm text-leaf-deep">共有旅を一覧から削除しました。</p> : null}
        {params.error === "leave" ? <p className="rounded-2xl border border-blossom bg-blossom-soft px-4 py-3 text-sm text-[#8f4c59]">削除できませんでした。時間をおいてもう一度お試しください。</p> : null}
...
function TripCard({ trip }: { trip: Trip }) {
  return <article className="rough-card p-4"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-leaf-soft text-leaf-deep"><IconUsers size={21} /></span><div className="min-w-0 flex-1"><h3 className="truncate font-bold">{trip.title}</h3><p className="mt-1 truncate text-xs text-ink-soft">{trip.member_names.join("・")}</p><p className="mt-2 flex items-center gap-1 text-[11px] text-ink-faint"><IconCalendar size={13} />更新 {formatDate(trip.updated_at.slice(0, 10))}・{trip.member_count}人</p></div><form action={leaveSharedTripAction}><input type="hidden" name="tripId" value={trip.trip_id} /><ConfirmSubmitButton message="この共有旅をあなたの一覧から削除します。他の参加者からは削除されません。よろしいですか？" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-ink-faint hover:bg-blossom-soft hover:text-[#8f4c59]" pendingLabel=""><IconTrash size={17} /></ConfirmSubmitButton></form></div></article>;
}
