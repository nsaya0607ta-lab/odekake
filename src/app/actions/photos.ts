...
import { TMP_RETENTION_HOURS, TMP_ROOT } from "@/lib/photos";
 ...
 * 画面を離れるとファイルだけが残る。自分の tmp/ 以下を見て、一定時間を
 * 過ぎたものを削除する。アプリを開いたときに1日1回だけ実行する。
 */
export async function cleanupTemporaryPhotosAction(): Promise<{ removed: number }> {
  const { supabase, user } = await requireUser();
  const root = `${TMP_ROOT}/${user.id}`;
  ...
  if (error || !entries) return { removed: 0 };
