 ...
 *   第4引数: それ以外の全キャッチ（わんこ・フラッド中のキャラ含む）に一律で適用される「通常時」キャッチ率。
 *     反応の遅れ等による平常時の取りこぼしを表す（2026-09-02〜、現実的プレイの標準前提。詳細はdocs/minigame-time-balance.md）。
 ...
 *   （代わりに第5引数の「密集時キャッチ率」で、同時多発時の取りこぼしを近似している）
 * - 「スポーンした物を必ずしも100%キャッチできるわけではない」現実的なプレイを、通常時/密集時/時間増加系の
 *   3段階キャッチ率で近似している（完全な位置判定・反応時間シミュレーションではない）
 */
...
const SCORE_MULT_DURATION_LR_SEC = Number(GAME_TSX.match(/const SCORE_MULT_DURATION_LR_SEC = (\d+);/)[1]);
const SCORE_MULT_DURATION_MR_SEC = Number(GAME_TSX.match(/const SCORE_MULT_DURATION_MR_SEC = (\d+);/)[1]);
...
const TIME_BONUS_IDS = new Set(["toy_duck_plush", "toy_carrot", "food_paw_melon_bread", "sushi_salmon", "sushi_buri", "interior_anball", "other_azuki", "other_omojii", "sushi_otoro", "summer_frenchie", "other_burebur"]);
...
const SPAWN_DYNAMICS_IDS = new Set(["toy_rainbow_ball", "interior_stretch_rod", "toy_treasure_puzzle", "other_xmas_party", "other_pondeomo", "other_pondear", "other_jare_a", "interior_ragby_ar", "other_listen_to_the_a", "other_mrs_green_apple", "sushi_shirasu"]);
...
const SCORE_MULT_IDS = new Set(["toy_meat", "interior_spring_flower_wreath", "other_kamunayo", "other_nisoku_a", "other_azubee", "interior_kinoko_azubee", "other_kobee", "interior_shikkoku_no_ar", "other_pink_omo", "other_narcissist_a", "other_mafia_a", "sushi_maguro_akami", "sushi_chutoro", "sushi_uni", "sushi_nama_ebi"]);
...
const NORMAL_ITEM_IDS = new Set([
  ...
  "sushi_kanpachi", "sushi_ebi", "sushi_aburi_saba", "sushi_anago",
...
function simulateOneRound(lv, catchAll, timeBonusCatchRate = 0.8, normalCatchRate = 0.85, denseCatchRate = 0.5) {
  ...
  function runItemSkillEffect(skillId, lvIdx) {
    ...
    switch (skillId) {
      ...
      case "toy_treasure_puzzle": {
        ...
        else if (outcome === "item_double") { multiplier15Until = t + SCORE_MULT_DURATION_SR_SEC * 1000; multiplier15Value = LV.TREASURE_DOUBLE_MULT[lvIdx]; }
      ...
      case "other_kamunayo": multiplier15Until = t + SCORE_MULT_DURATION_SSR_SEC * 1000; multiplier15Value = LV.KAMUNAYO_MULT[lvIdx]; break;
      case "summer_frenchie": addBonusTime(LV.SUMMER_ADD[lvIdx]); multiplier15Until = t + SCORE_MULT_DURATION_LR_SEC * 1000; multiplier15Value = LV.SUMMER_MULT[lvIdx]; break;
      case "interior_kinoko_azubee": multiplier15Until = t + SCORE_MULT_DURATION_UR_SEC * 1000; multiplier15Value = LV.KINOKO_SCORE[lvIdx]; break;
      ...
      case "other_kobee": points += LV.KOBEE_PT[lvIdx]; multiplier15Until = t + SCORE_MULT_DURATION_UR_SEC * 1000; multiplier15Value = LV.KOBEE_MULT[lvIdx]; break;
  ...
  while (t < endAt) {
    ...
    const highRarityLockActive = t < highRarityLockUntil;
    ...
    if (roll < dogWeight) {
      if (Math.random() < FRENCHIE_SKIN_SPAWN_CHANCE) {
        ...
        if (attemptCatch()) resolveCatch("dog", dogGoldenActive ? "toorematen_golden_dog" : null, null, 0);
      }
      continue;
    }
    roll -= dogWeight;
