module.exports = function previewItemCatchHazardsLoader(source) {
  ...
  const mustReplace = (from, to, label) => {
    if (!out.includes(from)) throw new Error("[preview hazards] source pattern not found: " + label);
  ...
  mustReplace(bagBlockEnd, hazardInsert, "spawn");
  ...
  mustReplace(bagCatchTail, hazardCatch, "catch");
