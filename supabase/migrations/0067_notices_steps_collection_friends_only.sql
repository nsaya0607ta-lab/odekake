-- =============================================================
-- 1万歩達成・図鑑レアアイテム獲得のお知らせをフレンド限定に変更
-- =============================================================
-- 0036_notices.sql では steps_10000 と collection_rare を全ユーザーに
-- 見せていたが、friend_spot / minigame_best と同様にフレンドにのみ
-- 見せるよう変更する。admin（運営からのお知らせ）は引き続き全員に見せる。
begin;
...
begin
  if to_regclass('public.notices') is null then
    raise exception using
      errcode = 'P0001',
      message = 'NOTICES_STEPS_COLLECTION_FRIENDS_ONLY_MIGRATION_MISSING_DEPENDENCY: public.notices',
      hint = 'お知らせ機能（0036_notices.sql）を先に適用してください。';
  end if;
  if to_regprocedure('public.is_friend(uuid)') is null then
    raise exception using
      errcode = 'P0001',
      message = 'NOTICES_STEPS_COLLECTION_FRIENDS_ONLY_MIGRATION_MISSING_DEPENDENCY: public.is_friend(uuid)',
...
select
  'NOTICES_STEPS_COLLECTION_FRIENDS_ONLY_READY'::text as status,
