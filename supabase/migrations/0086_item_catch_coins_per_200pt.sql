-- =============================================================
-- アイテムキャッチのコイン換算レートを「25pt=1コイン」→「200pt=1コイン」に変更
-- =============================================================
-- record_item_catch_result() が付与するコインは、これまでラウンドの
-- スコアを25で割った値（端数切り捨て、最低1コイン）だった。
-- これを200で割る（＝1コイン=200ポイント）に変更する。
...
begin
  if to_regprocedure('public.record_item_catch_result(text, integer, integer, integer, integer)') is null then
    raise exception using
      errcode = 'P0001',
      message = 'ITEM_CATCH_COINS_PER_200PT_MISSING_DEPENDENCY',
      hint = 'アイテムキャッチのラウンド時間50秒対応（0080_item_catch_round_seconds_50.sql）を先に適用してください。';
  end if;
end;
$$;
...
begin
  ...
  v_reward := greatest(1, p_score / 200) + p_bonus_coins;
...
select
  'ITEM_CATCH_COINS_PER_200PT_READY'::text as status,
