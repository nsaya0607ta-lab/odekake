...
Lv.4 以降でもらえるアクセサリーは、犬に実際に着せて表示します。
着ている姿はホームを歩き回る犬（`WanderingFrenchie`）とコイン画面のプレビューに出ます。

付け場所は4つあり、同じ場所のものはいちばん新しく解放したものだけを着けます
（`getEquippedGear()`）。首輪だけはバンダナの襟もとに重なる別枠なので、両方着けられます。

| 場所 | 報酬 |
| --- | --- |
| あたま | 麦わら帽子（Lv.18）→ キャプテン帽（Lv.27）→ 旅王のクラウン（Lv.30） |
| バンダナ | お花のバンダナ（Lv.6）→ 青空のバンダナ（Lv.15） |
| くびわ | 若葉の首輪（Lv.4）→ 夕焼けの首輪（Lv.11）→ 星空の首輪（Lv.21） |
| せなか | おでかけリュック（Lv.9）→ 探検家リュック（Lv.25） |

犬の1枚絵には、もともと帽子・バンダナ・リュックが描き込まれています。
別の装備をただ上に描くと下の緑がはみ出して貼り付けたように見えるため、
**元の装備を型紙にして、陰影と線画を残したまま色と飾りだけを差し替えています**。
型紙が元の装備と同じ形なので、はみ出しが起きません。

装備の絵は犬の絵と同じ 300×254 で、装備以外は透明です。アプリ側は同じ枠にそのまま
重ねるだけで、位置合わせの数値をひとつも持ちません。

```
public/characters/frenchie/gear/{装備}/{ポーズ}.webp   # 10装備 × 20ポーズ
```

作り直すとき（元の絵を差し替えたときなど）:

```bash
pip install pillow numpy scipy
python3 scripts/build-frenchie-gear.py --preview   # --preview で .preview/ に確認用の一覧が出る
```

型紙の切り分けはポーズごとに置いた種点で決めています（`scripts/frenchie_gear/masks.py` の
`SEEDS`）。新しいポーズを足したら、そこに帽子・首まわり・背中の座標を書き足してください。
...
- 残高はホーム右上のコインボタンに出ます。押すとコインの画面（`/mypage/coins`）が開き、
  所持コイン・いまの装備プレビュー・コインの使い道（ガチャ／ショップ）・もらい方・履歴が並びます。
...
- 付与量は `src/lib/coins.ts` と `supabase/migrations/0011_odekake_coins.sql` の両方に書いてあります。
  変えるときは必ず両方を揃えてください。
- コインを使う機能（ガチャ・ショップ）はまだありません。装備はレベルで解放され、コインは要りません。
  コイン画面のガチャ／ショップは
